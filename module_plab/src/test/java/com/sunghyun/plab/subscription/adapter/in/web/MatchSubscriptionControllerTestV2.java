...
class MatchSubscriptionControllerTest {
    ...
    private static final String BASE_REGISTER_URL = "/plab/subscription";

    @Test
    @DisplayName("구독 매치 등록 시, NotiValue 변환 확인하기")
    void regMatchSubscriptionWithNotEqualNotiType() throws Exception {
        //given
        MatchSubscriptionRegReqDto regDto = MatchSubscriptionRegReqDto.builder()
                .plabMatchNo(1L)
                .memberNo(1L)
                .notiType(NotiType.PLAYER_COUNT)
                .value(NotiSetting.MANAGER_FREE)
                .build()
                ;


        //when,then
        mockMvc.perform(post(BASE_REGISTER_URL)
    ...
    @Test
    @DisplayName("구독 매치 수정 시, NotiValue 타입 일치하는 경우 검증하기")
    void modMatchSubscriptionWithEqualNotiType() throws Exception {
        //given
        final Long subscriptionNo = 1L;
        final String url = BASE_REGISTER_URL+"/"+ subscriptionNo;
        MatchSubscriptionModReqDto modReqDto = MatchSubscriptionModReqDto.builder()
                .notiType(NotiType.PLAYER_COUNT)
                .value(NotiSetting.PLAYER_EIGHT)
                .build()
                ;
